...
def summary_spikes(total_neurons, sim_time, times, nidx, M, dict_key,
    ...
    figsize = fp.cm_to_inches([3, 8])

    fig, axs = plt.subplots(4, 1, figsize=figsize)
    # fig = plt.figure(figsize=figsize)  # width x height
    fig.set_constrained_layout_pads(w_pad=0, h_pad=0)

    # gs0 = gridspec.GridSpec(5, 1, hspace=0.2)
    
    # ax4 = plt.subplot(gs0[0, 0])  # Average fr
    # ax5 = plt.subplot(gs0[1, 0])  # ISI
    # ax6 = plt.subplot(gs0[2, 0])  # CV
    # ax7 = plt.subplot(gs0[3, 0])  # M
    # ax8 = plt.subplot(gs0[4, 0])  # Avalanches
    ax4 = axs[0]  # Average fr
    ax5 = axs[1]  # ISI
    ax6 = axs[2]  # CV
    ax7 = axs[3]  # M
    # ax8 = axs[4]  # Avalanches
...
if __name__ == '__main__':
    ...
    with open(filename, 'rb') as ff:
        ...
        plot_filename = 'Figure3_nw_summ2.png'
